...
bool CanDirectFetch(const Consensus::Params &consensusParams)
{
    ...
    if (fBCABootstrapping && consensusParams.BitcoinPostforkTime > 0) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                if (vGetData.size() > 0) {
                    ...
                    if (nodestate->fSupportsDesiredCmpctVersion &&
                        ...
                        pindexLast->pprev->IsValid(BLOCK_VALID_CHAIN) && !DISABLE_CMPCTBLOCK)
                    {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if(pfrom->fUsesAtomMagic) {
            LogPrintf("peer %d uses Atom magic in its headers\n", pfrom->id);
        }
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        const CBlockIndex *pindex = nullptr;
        CValidationState state;
        // When bootstrapping BCA network, continue even if there are invalid blocks.
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (pfrom->nVersion == 0) {
        if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(), CMessageHeader::MESSAGE_START_SIZE) == 0) {
            pfrom->fUsesAtomMagic = true;
        } else if (fBCABootstrapping) {
            // Allow to connect to Bitcoin clients when bootstrapping.
            pfrom->fUsesAtomMagic = false;
        }
    }
