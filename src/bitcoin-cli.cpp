...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-convertaddress=<bitcoin_address>", _("Convert a Bitcoin address into Bitcoin Atom address format."));
...
static int ConvertAddressFormat()
{
    ...
    CTxDestination addrDestination = DecodeDestinationWithParams(old_address, params);
    if (!IsValidDestination(addrDestination)) {
        fprintf(stderr, "Invalid Bitcoin address: %s\n", old_address.c_str());
        return EXIT_FAILURE;
    }
    fprintf(stdout, "%s\n", EncodeDestination(addrDestination).c_str());
    return EXIT_SUCCESS;
}
